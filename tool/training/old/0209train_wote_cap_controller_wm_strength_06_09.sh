...
# WoTE training script (controller-conditioned world model) — run twice:
#   1) controller_world_model_strength = 0.6
#   2) controller_world_model_strength = 0.9
#
...
run_one() {
  ...
  echo "  ctrl->wm     = enable=true strength=${wm_strength} target=${WM_INJECT_TARGET}"
...
# Run 1: strength=0.6
run_one 0.6 "${EXP_TAG_BASE}_${EXP_RUN_ID}_wm0p6"

# Run 2: strength=0.9
run_one 0.9 "${EXP_TAG_BASE}_${EXP_RUN_ID}_wm0p9"

echo "[DONE] Finished both runs."
