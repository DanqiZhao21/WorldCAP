...
# First WoTE + HERM finetune:
# - initialize WoTE from epoch=29-step=19950.ckpt
# - use frozen support-conditioned HERM
# - do not inject controller embedding/token into the latent world transition
# - run 4 GPUs for 32 epochs
...
export WOTE_TRAIN_PROFILE=herm_wm_reward_only
...
RUN_NAME=${RUN_NAME:-"wote_herm_exec_only_4gpu_32ep_${RUN_TS}"}

export WOTE_WANDB_PROJECT=${WOTE_WANDB_PROJECT:-WOTE-training-2}
export WOTE_WANDB_GROUP=${WOTE_WANDB_GROUP:-"wote-herm-exec-only-${RUN_TS}"}
export WOTE_WANDB_RUN_NAME="${RUN_NAME}"
export WOTE_WANDB_TAGS=${WOTE_WANDB_TAGS:-wote,herm,exec-only,no-controller-token,4gpu,32ep}
...
"${PYTHON}" ./navsim/planning/script/run_training.py \
  ...
  split=trainval \
  experiment_name="WoTE/herm_exec/herm_exec_only_4gpu_32ep_${RUN_TS}" \
