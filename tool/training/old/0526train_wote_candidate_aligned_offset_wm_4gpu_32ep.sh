...
# Candidate-aligned WoTE/WorldCAP finetune.
# Core idea:
#   raw anchors -> predicted offset -> active candidate trajectories
#   active candidates condition the world model, future BEV target composition,
#   and imitation-reward soft labels. HERM is intentionally disabled here.
...
RUN_NAME=${RUN_NAME:-"wote_candidate_aligned_offset_wm_4gpu_32ep_${RUN_TS}"}

export WOTE_WANDB_PROJECT=${WOTE_WANDB_PROJECT:-WOTE-training-2}
export WOTE_WANDB_GROUP=${WOTE_WANDB_GROUP:-"wote-candidate-aligned-offset-wm-${RUN_TS}"}
export WOTE_WANDB_RUN_NAME="${RUN_NAME}"
export WOTE_WANDB_TAGS=${WOTE_WANDB_TAGS:-wote,worldcap,candidate-aligned,offset-candidates,attn-film,no-herm,4gpu,32ep}
...
"${PYTHON}" ./navsim/planning/script/run_training.py \
  ...
  split=trainval \
  experiment_name="WoTE/candidate_aligned/offset_wm_4gpu_32ep_${RUN_TS}" \
