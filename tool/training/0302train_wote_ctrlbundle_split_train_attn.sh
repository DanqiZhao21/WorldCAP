...
#   GPU_IDS=0,1,2,3        -> sets CUDA_VISIBLE_DEVICES explicitly (highest priority)
#   CUDA_VISIBLE_DEVICES=0 -> also supported (kept if GPU_IDS is unset)
...
# Supports a special suffix `_s0` to inject controller style ONLY at world-model step 0.
# Examples:
#   RUN_FUSIONS=attn,film03
#   RUN_FUSIONS=film06,film09,attn_s0
RUN_FUSIONS=${RUN_FUSIONS:-"attn,film03,film06,film09,attn_s0"}
...
preflight_one() {
  ...
  local inject_first_only="$1"; shift
...
cfg.controller_world_model_inject_first_step_only = (str(r"""${inject_first_only}""").strip().lower() == "true")
...
print('[INFO] Preflight OK:', 'fusion='+fusion_s, 'strength='+str(cfg.controller_world_model_strength), 'first_step_only='+str(cfg.controller_world_model_inject_first_step_only))
...
run_one() {
  ...
  echo "  ctrl->wm     = enable=true fusion=${fusion} strength=${strength} target=${WM_INJECT_TARGET} first_step_only=${inject_first_only}"
  ...
  "${PYTHON}" ./navsim/planning/script/run_training.py \
    ...
    ++agent.config.controller_world_model_inject_first_step_only=${inject_first_only} \
...
for _fusion in "${_fusions[@]}"; do
  ...
  _inject_first_only="false"
  _base_fusion="${_fusion}"
  if [[ "${_fusion}" == *_s0 ]]; then
    _inject_first_only="true"
    _base_fusion="${_fusion%_s0}"
  fi

  _strength="${WM_STRENGTH_ATTN}"
  if [[ "${_base_fusion}" == add* ]]; then
    _strength="${WM_STRENGTH_ADD}"
  elif [[ "${_base_fusion}" == film06* ]]; then
    _strength="0.6"
  elif [[ "${_base_fusion}" == film09* ]]; then
    _strength="0.9"
  elif [[ "${_base_fusion}" == film* ]]; then
    _strength="${WM_STRENGTH_FILM}"
  fi

  preflight_one "${_base_fusion}" "${_strength}" "${_inject_first_only}"
  run_one "${EXP_TAG_BASE}_${_fusion}_${EXP_RUN_ID}" "${_base_fusion}" "${_strength}" "${_inject_first_only}"
done
