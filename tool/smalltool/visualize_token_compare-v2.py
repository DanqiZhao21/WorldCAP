...
def _plot_bev_focus_compare(
    *,
    ...
    # GT
    add_trajectory_to_bev_ax(ax, human_traj, TRAJECTORY_CONFIG["human"])

    # Baseline / Film: reuse provided agent style, differentiate by linestyle/linewidth only.
    baseline_cfg = dict(TRAJECTORY_CONFIG["agent"])
    baseline_cfg["line_style"] = "--"
    baseline_cfg["line_width"] = float(baseline_cfg.get("line_width", 2.0))
    baseline_cfg["line_color_alpha"] = 0.9
    baseline_cfg["marker_size"] = 4

    film_cfg = dict(TRAJECTORY_CONFIG["agent"])
    film_cfg["line_style"] = "-"
    film_cfg["line_width"] = float(film_cfg.get("line_width", 2.0)) + 1.0
    film_cfg["line_color_alpha"] = 1.0
    film_cfg["marker_size"] = 5

    add_trajectory_to_bev_ax(ax, baseline_traj, baseline_cfg)
    add_trajectory_to_bev_ax(ax, film_traj, film_cfg)

    # Minimal score box (only totals)
    ...
    lines = [
        "PDM Total",
        f"baseline: {_fmt(baseline_score)}",
        f"film:     {_fmt(film_score)}",
        "(baseline=--  film=-)",
    ]
    ax.text(
        0.02,
...
def main() -> int:
    ...
    with _hydra_init_ctx(config_dir):
        ...
        # Focus compare plot (single image): GT + baseline-selected + film-selected.
        focus_name = f"{token}_focus_compare.png"
        focus_path = _plot_bev_focus_compare(
    ...
    print(f"token: {token}")
    print(f"focus image:    {focus_path}")
