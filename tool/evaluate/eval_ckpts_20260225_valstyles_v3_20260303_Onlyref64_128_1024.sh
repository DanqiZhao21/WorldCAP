#!/usr/bin/env bash
# EVAL_CPU_THREADS=null EVAL_RAY_THREADS_PER_NODE=null bash /home/zhaodanqi/clone/WoTE/tool/evaluate/eval_ckpts_20260225_valstyles_v3_20260303_ref64_128_1024.sh
set -euo pipefail

# Eval script to test controller feature extractor generalization when the
# controller bundle size used at inference differs (64/128/1024), while keeping
# the model checkpoint fixed.
#
# Requirement:
# - Evaluate ATTN fusion only.
# - Same ckpt for all runs.
# - During eval, provide different CTRL_REF/CTRL_EXEC bundle sizes (64/128/1024)
#   to see whether it generalizes.
# - Planning anchors (PLANNER_ANCHORS/SIM_REWARD_DICT) stay the same for all runs.
#
# Mode:
# - Fixed style idx: each run uses one simulator/controller style index.
#
# Outputs:
...
# Shared checkpoint (same ckpt for all bundle-size inference runs)
CKPT_SHARED=${CKPT_SHARED:-"/home/zhaodanqi/clone/WoTE/trainingResult/ckpts_20260225_051808/epoch=39-step=53200.ckpt"}

# Back-compat vars (all point to the shared ckpt)
CKPT_ATTN_CTRL64=${CKPT_ATTN_CTRL64:-"${CKPT_SHARED}"}
CKPT_ATTN_CTRL128=${CKPT_ATTN_CTRL128:-"${CKPT_SHARED}"}
CKPT_ATTN_CTRL1024=${CKPT_ATTN_CTRL1024:-"${CKPT_SHARED}"}
...
OUT_ROOT=${OUT_ROOT:-"eval/WoTE/ckpt_20260303/attn_sharedckpt_inferbundle64_128"}
...
preflight() {
  ...
  for f in \
    ...
    "${PLANNER_ANCHORS}" \
    "${SIM_REWARD_DICT}" \
    "${CTRL_REF_64}" "${CTRL_EXEC_64}" \
    "${CTRL_REF_128}" "${CTRL_EXEC_128}" \
    "${CTRL_REF_1024}" "${CTRL_EXEC_1024}" \
    "${CKPT_ATTN_CTRL64}" "${CKPT_ATTN_CTRL128}" "${CKPT_ATTN_CTRL1024}";
  do
    if [[ ! -f "${f}" ]]; then
  ...
  echo "[INFO] CTRL_EXEC_64=${CTRL_EXEC_64}"
  echo "[INFO] CTRL_EXEC_128=${CTRL_EXEC_128}"
  echo "[INFO] CTRL_EXEC_1024=${CTRL_EXEC_1024}"
  ...
  local b
  for b in "${CTRL_EXEC_64}" "${CTRL_EXEC_128}" "${CTRL_EXEC_1024}"; do
    echo "[INFO] Preflight bundle: ${b}"
    CTRL_EXEC="${b}" \
...
main() {
  ...
  echo "============================================================"
  echo "[EVAL] attn-only (inference bundle size ablation: 64/128/1024)"
  echo "  CKPT_SHARED=${CKPT_SHARED}"
  echo "  OUT_ROOT=${NAVSIM_EXP_ROOT}/${OUT_ROOT}"
  echo "  planning anchors: ${PLANNER_ANCHORS} (constant)"
  ...
  # Fixed styles: for each style idx, run 3 evals with matched controller bundles.
  for s in "${TRK_STYLE_IDXS[@]}"; do
    # ctrl64
    CTRL_REF="${CTRL_REF_64}" CTRL_EXEC="${CTRL_EXEC_64}" \
      run_fixed_style "${s}" "ctrl64_attn" "${CKPT_ATTN_CTRL64}" 1 attn 0.0 false

    # ctrl128
    CTRL_REF="${CTRL_REF_128}" CTRL_EXEC="${CTRL_EXEC_128}" \
      run_fixed_style "${s}" "ctrl128_attn" "${CKPT_ATTN_CTRL128}" 1 attn 0.0 false

    # # ctrl1024
    # CTRL_REF="${CTRL_REF_1024}" CTRL_EXEC="${CTRL_EXEC_1024}" \
    #   run_fixed_style "${s}" "ctrl1024_attn" "${CKPT_ATTN_CTRL1024}" 1 attn 0.0 false
  done
