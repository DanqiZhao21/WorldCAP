...
SPLIT=${SPLIT:-test}
OUT_BASE=${OUT_BASE:-eval/hard_scene_composite_once_20260513}
...
run_eval() {
  ...
  local ckpt="$2"
  local gpu="$3"
  local scene_filter="navtest_hard_composite_p90"
  ...
  if find "${out_dir}" -maxdepth 1 -name '*.csv' ! -name 'partial_*.csv' -type f 2>/dev/null | grep -q .; then
    echo "[SKIP] ${tag}: final CSV already exists in ${out_dir}"
    return 0
  fi
...
run_eval "epoch19_step21280" "${CKPT_NEW}" 0 &
run_eval "epoch29_step19950" "${CKPT_BASE}" 1 &
wait
