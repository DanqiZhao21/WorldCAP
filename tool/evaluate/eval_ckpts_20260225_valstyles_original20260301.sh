...
#FIXME:
#ablation1 和 anlation2
# CKPT_BASELINE=${CKPT_BASELINE:-"${ROOT}/trainingResult/epoch=29-step=19950.ckpt"}
CKPT_ATTN_FIRSTSTEP=${CKPT_ATTN:-"${ROOT}/trainingResult/ckpts_20260225_003446/epoch=39-step=53200.ckpt"}
CKPT_FILM06=${CKPT_FILM03:-"${ROOT}/trainingResult/ckpts_20260225_051808/epoch=39-step=53200.ckpt"}
CKPT_FILM09=${CKPT_FILM03:-"${ROOT}/trainingResult/ckpts_20260225_051808/epoch=39-step=53200.ckpt"}
...
OUT_ROOT=${OUT_ROOT:-"eval/WoTE/ckpt_20260228/Ablation2InjectDepth"}
...
preflight() {
  ...
  local f
  for f in "${SCRIPT}" "${CKPT_BASELINE}" "${CKPT_ATTN}" "${CKPT_FILM03}" "${CTRL_REF}" "${CTRL_EXEC}"; do
    if [[ ! -f "${f}" ]]; then
...
run_fixed_style() {
  ...
  run_eval "${ckpt_path}" "${OUT_ROOT}/${style_idx}/${model_tag}" "${model_tag}" "${ctrl_enable}" "${wm_fusion}" "${wm_strength}"
}
...
main() {
  ...
  for s in "${TRK_STYLE_IDXS[@]}"; do
    run_fixed_style "${s}" default "${CKPT_BASELINE}" 0 attn 0.0
    run_fixed_style "${s}" attn "${CKPT_ATTN}" 1 attn 0.0
    run_fixed_style "${s}" film03 "${CKPT_FILM03}" 1 film03 "${FILM_WM_STRENGTH}"
  done
