...
def initialize_ray(
    ...
    if master_node_ip and use_distributed:
        ...
        try:
            ...
            # If Ray auto-connects to an existing cluster, resource args must not be provided.
            msg = str(e)
            if 'connecting to an existing cluster' in msg or 'must not be provided' in msg:
                logger.info('Existing Ray cluster detected; reconnecting with address=auto and without resource args.')
                ray.init(
                    address='auto',
                    dashboard_host='0.0.0.0',
                    local_mode=local_mode,
