...
#         # normalize and fill progress values进度
...
#             #哪条 proposal 的 ego polygon 和哪个 object polygon 相交
...
class PDMScorer:
    ...
    def __init__(
        ...
        self._collision_time_idcs: Optional[npt.NDArray[np.float64]] = None
        self._ttc_time_idcs: Optional[npt.NDArray[np.float64]] = None
        # store driving-direction underlying metric for visualization
        self._oncoming_progress_value: Optional[npt.NDArray[np.float64]] = None
    ...
    def _reset(
        ...
        self._collision_time_idcs.fill(np.inf)
        self._ttc_time_idcs.fill(np.inf)
        # initialize oncoming progress value
        self._oncoming_progress_value = np.zeros(self._num_proposals, dtype=np.float64)
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        # aggregate progress over a sliding horizon per proposal
        # shape -> (num_proposals, num_poses+1)
        oncoming_progress_over_horizon = np.stack(
            [
                oncoming_progress[:, max(0, time_idx - horizon) : time_idx + 1].sum(axis=-1)
                for time_idx in range(self.proposal_sampling.num_poses + 1)
            ],
            axis=-1,
        )

        # store the max oncoming progress per proposal for visualization
        self._oncoming_progress_value = oncoming_progress_over_horizon.max(axis=-1)

        for proposal_idx, progress in enumerate(self._oncoming_progress_value):
            if progress < self._config.driving_direction_compliance_threshold:
                driving_direction_compliance_scores[proposal_idx] = 1.0
            elif progress < self._config.driving_direction_violation_threshold:
                driving_direction_compliance_scores[proposal_idx] = 0.5
