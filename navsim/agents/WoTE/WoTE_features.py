...
class WoTEFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        # Robustly handle lidar_pc shape being either (C, N) or (N, C)
        _lidar = agent_input.lidars[-1].lidar_pc
        _lidar = np.asarray(_lidar)
        if _lidar.ndim != 2:
            raise ValueError("lidar_pc must be a 2D array")
        # Channels-first (C, N) e.g., 6 x num_points
        if _lidar.shape[0] <= _lidar.shape[1]:
            lidar_pc = _lidar[:3].T
        else:
            # Points-first (N, C) e.g., num_points x 6
            lidar_pc = _lidar[:, :3]
