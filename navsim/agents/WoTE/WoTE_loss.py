...
#     # offset reward
#     # Prefer executed predicted trajectories (controller-aware) if provided by the model.
#     trajectory_anchors = predictions.get("trajectory_offset_exec_trajs", None)
...
#     # Prefer controller-executed residual offset if present.
#     predicted_offsets = predictions.get("trajectory_offset_executed", None)
...
#     # Calculate L2 distance between each candidate trajectory and the target trajectory.
#     # Supports:
#     #   - trajectory_anchors: [K,T,3]
#     #   - trajectory_anchors: [B,K,T,3]
#     if trajectory_anchors.dim() == 4:
#         # [B,K,T,3]
#         anchors_flat = trajectory_anchors.reshape(Bz, trajectory_anchors.shape[1], -1).to(target_trajectory.device)
#     else:
#         # [K,T,3]
#         num_trajs = trajectory_anchors.shape[0]
#         anchors_flat = trajectory_anchors.reshape(num_trajs, -1).unsqueeze(0).repeat(Bz, 1, 1).to(target_trajectory.device)

#     l2_distances = torch.cdist(anchors_flat, target_trajectory, p=2)  # Shape: [batch_size, K, 1]
