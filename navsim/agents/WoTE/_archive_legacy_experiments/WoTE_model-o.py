...
class WoTEModel(nn.Module):
    def __init__(self, 
        ...
        hidden_dim = 256
        NUM_CLUSTERS = config.n_clusters if hasattr(config, 'n_clusters') else 256#轨迹聚类
        ...
        print("💚💚💚cluster_file_path: ", cluster_file)
        ...
        if self.use_agent_loss:
            ...
            agent_tf_decoder_layer = nn.TransformerDecoderLayer(#yTorch 的 nn.TransformerDecoderLayer 结构 包含三个部分：Self-Attention Cross-Attention FFN (Feedforward)
                d_model=config.tf_d_model,
