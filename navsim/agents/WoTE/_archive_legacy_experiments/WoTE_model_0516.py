...
class WoTEModel(nn.Module):
    def __init__(self, 
        ...
        # - 'attn': learned attention pooling (default) && # - 'mean': average over controller-bank candidates
        #💗💗controller bank/style 汇聚层,从 controller bank 的多个 controller embedding 里，用可学习 attention 权重汇聚出一个 global style embedding
        ...
        #💗💗world model latent transition 注入层
    ...
    def extract_trajectory_feature(self, features: Dict[str, torch.Tensor], targets=None) -> Dict[str, Any]:
        ...
        rank_only = bool(getattr(self, '_external_rank_only', False))
        ...
        if self.is_eval:
            ...
            if rank_only:
                offseted_trajectory_anchors = init_trajectory_anchor
            else:
                offseted_trajectory_anchors = init_trajectory_anchor + trajectory_offset
            planned_trajectory_anchors = offseted_trajectory_anchors
            ...
            if bool(getattr(self, "herm_enable", False)) and bool(getattr(self, "herm_apply_in_train", False)):
                scored_trajectory_anchors = self._execute_candidates_with_herm(init_trajectory_anchor)
    ...
    def extract_reward_feature(self, trajectory_outputs, targets) -> Dict[str, torch.Tensor]:
        ...
        # Multiple iterations
        num_iterations = self.num_fut_timestep# num_fut_timestep=1
        ...
        if ctrl_enabled and (not first_step_only):
            if fusion in {'attn', 'attention', 'cross_attn', 'cross_attention', 'attn_film', 'attention_film', 'cross_attn_film', 'cross_attention_film'}:
...
#ADD 剩下的都是一些功能函数
    ...
    def _compute_controller_style_token(self, batch_size: int, device: torch.device) -> torch.Tensor:
        ...
        #controller style embedding [B, 64] -> world model token [B, 256]
        当controller_world_model_fusion = "film" 或者"add"的时候使用
    ...
    def _compute_controller_bank_tokens(self, batch_size: int, num_traj: int, device: torch.device) -> torch.Tensor:
        ...
        N_ctrl controller trajectory embeddings -> N_ctrl controller tokens.
        controller_world_model_fusion = "attn"时候调用
    ...
    def _compute_controller_style_emb64(self, batch_size: int, device: torch.device) -> torch.Tensor:
        ...
        N_ctrl controller trajectory embeddings -> one controller style embedding.
        Returns:
    ...
    def _latent_world_model_processing(
        ...
        if getattr(self._config, 'controller_condition_on_world_model', getattr(self, 'controller_condition_on_world_model', False)):
            ...
            elif fusion in {'attn_film', 'attention_film', 'cross_attn_film', 'cross_attention_film'}:
                # Per-token FiLM conditioned by controller-bank cross-attention context.
                bank_tokens = controller_bank_tokens
                if bank_tokens is None:
                    bank_tokens = self._compute_controller_bank_tokens(batch_size, num_traj, scene_feature.device)  # [B*num_traj, N_ctrl, 256]
